...
默认字数目标：
- 常规章节默认 3500-4000 字（除非大纲或用户另有指定）。
...
- ✅ 抽象判断 → 动作/反应/代价
- ✅ 删除”总结式旁白”和大段纯解释
- ✅ **白描优先**：用动作、对话、场景传递情绪，禁止用比喻解释感受（如”胃里像被人捏了一下””心里像被石头压着”）。情绪通过生理反应+行为呈现，让读者自己感受
...
- 把”像/仿佛/宛如+感受解释”改为直接白描：删除比喻词，用动作或感官事实替代。例：”心里像被石头压着”→”胸口发闷，他深吸了两口气才缓过来”。
...
| 长句比例 | >40字的句子 <10% | 拆分长句 |
| 比喻密度 | 无"像+感受解释"句式，每500字比喻标记≤1 | 删除或改为动作白描 |
| 解释段 | 无连续>200字的纯解释 | 打散或删除 |

## 章节类型适配（按大纲，不按字数）
- **常规推进章（默认 3500-4000字）**: 执行全部硬约束 + 软建议优先落实
- **过渡章（按大纲判定）**:
  - 冲突导入可放宽到前 400 字
