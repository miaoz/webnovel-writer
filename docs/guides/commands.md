...
根据用户修改意见对已提交章节做受控修订。表达、文风、节奏类修改可以直接修正文；人物状态、事件结果、道具归属、时间地点、关系变化、伏笔方向、章末结局等事实变化必须先生成大纲修订提案，用户确认并刷新 runtime contracts 后再改正文。
...
Codex 中 `/webnovel-write`、`/webnovel-review`、`/webnovel-revise`、`/webnovel-amend` 默认按 `../../references/codex/agent-protocols.md` 在当前会话执行 inline protocol，并生成与 Claude Agent 相同的 JSON artifacts。
